    ...
    def __init__(self, certainty: float = 0, class_index: int = -1, class_name: str = '', instance_name: str = '',
        ...
        if serialised_object is not None:
            ...
            if self.class_name == 'RightHand':
                self.class_index = ac.objects.index('right_hand')
            elif self.class_name == 'LeftHand':
                self.class_index = ac.objects.index('left_hand')
...
class SceneGraph:
    ...
    def check_integrity(self):
        assert self.right_action is None or (0 <= self.right_action < len(ac.actions))
        assert self.left_action is None or (0 <= self.left_action < len(ac.actions))
    ...
    def to_data_dict(self, mirrored=False) -> Dict[str, List]:
        ...
        graph_globals: List[float] = one_hot_encode(len(ac.actions), [] if action_id is None else [action_id])

        graph_nodes: List[List[float]] = []
        for node_id, (class_id, _, bb) in self.nodes.items():
            if mirrored:
                class_id = SceneGraph._mirror_hands(class_id)
            graph_nodes.append(one_hot_encode(len(ac.objects), [class_id]) + list(bb))
        ...
        for (sender, receiver), relations in self.edges.items():
            ...
            graph_edges.append(one_hot_encode(len(ac.relations), relations))
    ...
    def _mirror_hands(class_id: int) -> int:
        if ac.objects[class_id] == 'right_hand':
            return ac.objects.index('left_hand')
        if ac.objects[class_id] == 'left_hand':
            return ac.objects.index('right_hand')
        return class_id

    @staticmethod
    def _mirror_relations(relation_id: int) -> int:
        if ac.relations[relation_id] == 'left of':
            return ac.relations.index('right of')
        if ac.relations[relation_id] == 'right of':
            return ac.relations.index('left of')
        return relation_id
        ...
        for node_id, (_, node_name, _) in sg.nodes.items():
            for past_node_id, (_, past_node_name, _) in past_sg.nodes.items():
                if node_name == past_node_name:
                    ...
                    temporal_sg.edges[key] = [ac.relations.index('temporal')]
    ...
    def check_integrity(self) -> None:
        ...
        for frame in range(0, self.frame_count, 1):
            ...
            for obj in self.objects[frame]:
                assert 0 <= obj.class_index < len(ac.objects)
    ...
    def to_scene_graph(self, frame_number: int) -> SceneGraph:
        ...
        for relation in self.relations[frame_number]:
            ...
            if key not in sg.edges:
                sg.edges[key] = []
            sg.edges[key].append(ac.relations.index(relation.relation_name))
...
class SceneGraphProxy:
    ...
    def load(self):
        with open(self.path, 'rb') as f:
            try:
                ...
                ac.print1('Error while unpickling `{}`: {}.  Retrying.'.format(self.path, e))
                time.sleep(0.1)
                try:
                    return getattr(self, 'load_{}'.format(self.mode))(pickle.load(f))
                except pickle.UnpicklingError:
                    ac.print0('Repeatedly failed to unpickle `{}`'.format(self.path))
    ...
    def load_contact(self, graph):
        ...
        for i in range(len(graph['edges'])):
            for j, rel in enumerate(ac.relations):
...
def generate_dataset(basepath: str, config: str, history_size: int) -> None:
    ...
    for subject, task, take in crawl_dataset():
        ac.print1('Generating graphs data for {} | {} | {}.'.format(subject, task, take))
