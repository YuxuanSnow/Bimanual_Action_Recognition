...
def main(args):
    ...
    logger = Utils.logutil.Logger(os.path.join(args.log_root, timestamp))
    # configure log directory: specified by timestamp

    # Load data
    training_set, valid_set, testing_set, train_loader, valid_loader, test_loader, img_index = utils.get_hico_data(args)
    # load the data with given batch size
    ...
    model = Model.GNN_IMG(model_args) # construct GPNN structure for hico dataset
